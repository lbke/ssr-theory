...
2. Compute the props of the template based on user's request. This step simply translates the HTTP request into a set of attributes that makes more sense in the business domain. Technically, this is not needed for server-side rendering, the request could be directly passed to the render function. However, modern framework most often have an hybrid approach combining
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYzMzA4MjU0MSwtMjg0NTM5MTQ4LC0zNT

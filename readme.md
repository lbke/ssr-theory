# Theoretical foundations for server-rendering and static-rendering
...
Therefore, the page is entirely defined by the choice of the template and by the filling values.
...
function buildTimeRender = (buildTimeProps: Array<Props>, render: Renderer) => {
    return buildTimeProps.map(render)
}
